...
tl_tensor* add_bias(tl_tensor* x, tl_tensor* bias) {
    ...
    for (int i = 0; i < x->dims[0]; i++) {  /* for each batch */
        for (int j = 0; j < x->dims[1]; j++) {  /* for each feature */
            ...
            TL_TENSOR_DATA_TO(bias, j, b_val, TL_FLOAT);
            x_val += b_val;
...
int main() {
    ...
    /* Create tensors */
    tl_tensor* X = tl_tensor_create(input_data, 2, (int[]){1, INPUT_SIZE}, TL_FLOAT);
    tl_tensor* W1 = tl_tensor_create(W1_data, 2, (int[]){INPUT_SIZE, HIDDEN_SIZE}, TL_FLOAT);
    tl_tensor* b1 = tl_tensor_create(b1_data, 1, (int[]){HIDDEN_SIZE}, TL_FLOAT);
    tl_tensor* W2 = tl_tensor_create(W2_data, 2, (int[]){HIDDEN_SIZE, OUTPUT_SIZE}, TL_FLOAT);
    tl_tensor* b2 = tl_tensor_create(b2_data, 1, (int[]){OUTPUT_SIZE}, TL_FLOAT);
    ...
    tl_tensor* h1_bias = add_bias(h1, b1);
    ...
    tl_tensor* output = tl_tensor_matmul(h1_bias, W2, NULL);
    printf("  After matmul: [%d, %d]\n", output->dims[0], output->dims[1]);

    tl_tensor* final_output = add_bias(output, b2);
