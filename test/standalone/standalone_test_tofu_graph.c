...
void test_graph_input_node()
{
    ...
    tofu_tensor* t = tofu_tensor_create(data, 2, (int[]){2, 2}, TOFU_FLOAT);

    /* Add input node */
    tofu_graph_node* x = tofu_graph_input(g, t);
    assert(x != NULL);
    ...
    /* Verify value access */
    tofu_tensor* val = tofu_graph_get_value(x);
    assert(val == t);
...
void test_graph_param_node()
{
    ...
    /* Add parameter node */
    tofu_graph_node* W = tofu_graph_param(g, t);
    assert(W != NULL);
    assert(W->op == TOFU_OP_PARAM);
    assert(W->value == t);
...
void test_graph_multiple_nodes()
{
    ...
    tofu_tensor* t1 = tofu_tensor_create(data1, 1, (int[]){2}, TOFU_FLOAT);
    tofu_tensor* t2 = tofu_tensor_create(data2, 1, (int[]){2}, TOFU_FLOAT);
    tofu_tensor* t3 = tofu_tensor_create(data3, 1, (int[]){2}, TOFU_FLOAT);

    tofu_graph_node* x = tofu_graph_input(g, t1);
    tofu_graph_node* W = tofu_graph_param(g, t2);
    tofu_graph_node* b = tofu_graph_param(g, t3);
...
void test_graph_capacity_expansion()
{
    ...
    tofu_tensor* dummy = tofu_tensor_zeros(1, (int[]){1}, TOFU_FLOAT);

    for (int i = 0; i < initial_capacity + 10; i++) {
        tofu_graph_node* node = tofu_graph_input(g, dummy);
        assert(node != NULL);
        assert(node->id == i);
    }
    ...
    tofu_tensor_free_data_too(dummy);
    tofu_graph_free(g);
    printf("  ✓ PASSED\n");
}
...
void test_graph_zero_grad()
{
    ...
    W->grad = tofu_tensor_zeros(1, (int[]){4}, TOFU_FLOAT);
    float grad_val = 5.0f;
    for (int i = 0; i < 4; i++) {
        TOFU_TENSOR_DATA_FROM(W->grad, i, grad_val, TOFU_FLOAT);
    }

    /* Verify gradient is set */
    float check_val;
    TOFU_TENSOR_DATA_TO(W->grad, 0, check_val, TOFU_FLOAT);
...
void test_graph_matmul()
{
    ...
    tofu_tensor* t_a = tofu_tensor_create(data_a, 2, (int[]){2, 3}, TOFU_FLOAT);
    tofu_tensor* t_b = tofu_tensor_create(data_b, 2, (int[]){3, 2}, TOFU_FLOAT);
...
void test_graph_add()
{
    ...
    tofu_tensor* t_a = tofu_tensor_create(data_a, 1, (int[]){3}, TOFU_FLOAT);
    tofu_tensor* t_b = tofu_tensor_create(data_b, 1, (int[]){3}, TOFU_FLOAT);

    tofu_graph_node* a = tofu_graph_input(g, t_a);
    tofu_graph_node* b = tofu_graph_param(g, t_b);
    tofu_graph_node* c = tofu_graph_add(g, a, b);

    assert(c != NULL);
    assert(c->op == TOFU_OP_ADD);
    assert(c->value != NULL);
...
void test_graph_relu()
{
    ...
    tofu_graph_node* x = tofu_graph_param(g, t);
    tofu_graph_node* y = tofu_graph_relu(g, x);

    assert(y != NULL);
    assert(y->op == TOFU_OP_RELU);
...
void test_graph_softmax()
{
    ...
    tofu_graph_node* x = tofu_graph_input(g, t);
    tofu_graph_node* y = tofu_graph_softmax(g, x, 0);

    assert(y != NULL);
    assert(y->op == TOFU_OP_SOFTMAX);
...
void test_graph_composite()
{
    ...
    tofu_tensor* t_x = tofu_tensor_create(data_x, 1, (int[]){2}, TOFU_FLOAT);
    tofu_tensor* t_W = tofu_tensor_create(data_W, 2, (int[]){2, 3}, TOFU_FLOAT);
    tofu_tensor* t_b = tofu_tensor_create(data_b, 1, (int[]){3}, TOFU_FLOAT);
    ...
    tofu_graph_node* xWb = tofu_graph_add(g, xW, b);
    tofu_graph_node* y = tofu_graph_relu(g, xWb);
...
void test_backward_simple_matmul()
{
    ...
    tofu_tensor* t_x = tofu_tensor_create(data_x, 1, (int[]){2}, TOFU_FLOAT);
    tofu_tensor* t_W = tofu_tensor_create(data_W, 2, (int[]){2, 1}, TOFU_FLOAT);
    ...
    float grad_y;
    TOFU_TENSOR_DATA_TO(y->grad, 0, grad_y, TOFU_FLOAT);
    ...
    for (int i = 0; i < 2; i++) {
        float grad;
        TOFU_TENSOR_DATA_TO(W->grad, i, grad, TOFU_FLOAT);
...
void test_backward_add()
{
    ...
    tofu_tensor* t_x = tofu_tensor_create(data_x, 1, (int[]){3}, TOFU_FLOAT);
    tofu_tensor* t_y = tofu_tensor_create(data_y, 1, (int[]){3}, TOFU_FLOAT);
    ...
    for (int i = 0; i < 3; i++) {
        float grad_x, grad_y;
        TOFU_TENSOR_DATA_TO(x->grad, i, grad_x, TOFU_FLOAT);
        TOFU_TENSOR_DATA_TO(y->grad, i, grad_y, TOFU_FLOAT);
...
void test_backward_relu()
{
    ...
    for (int i = 0; i < 5; i++) {
        float grad;
        TOFU_TENSOR_DATA_TO(x->grad, i, grad, TOFU_FLOAT);
...
void test_gradient_accumulation()
{
    ...
    tofu_graph_node* x = tofu_graph_param(g, t);
    tofu_graph_node* z = tofu_graph_add(g, x, x);
...
void test_backward_layer_norm()
{
    ...
    tofu_tensor* t_x = tofu_tensor_create(data_x, 2, (int[]){2, 3}, TOFU_FLOAT);
    tofu_tensor* t_gamma = tofu_tensor_create(data_gamma, 1, (int[]){3}, TOFU_FLOAT);
    tofu_tensor* t_beta = tofu_tensor_create(data_beta, 1, (int[]){3}, TOFU_FLOAT);
...
void test_optimizer_sgd()
{
    ...
    tofu_tensor* t_x = tofu_tensor_create(data_x, 1, (int[]){1}, TOFU_FLOAT);
    tofu_tensor* t_w = tofu_tensor_create(data_w, 1, (int[]){1}, TOFU_FLOAT);
    tofu_tensor* t_b = tofu_tensor_create(data_b, 1, (int[]){1}, TOFU_FLOAT);
    ...
    /* Create optimizer */
    tofu_optimizer* opt = tofu_optimizer_sgd_create(g, 0.1);  /* lr = 0.1 */
    assert(opt != NULL);
    ...
    TOFU_TENSOR_DATA_TO(w->value, 0, w_before, TOFU_FLOAT);
    TOFU_TENSOR_DATA_TO(b->value, 0, b_before, TOFU_FLOAT);
    ...
    TOFU_TENSOR_DATA_TO(w->value, 0, w_after, TOFU_FLOAT);
    TOFU_TENSOR_DATA_TO(b->value, 0, b_after, TOFU_FLOAT);
    ...
    float w_grad, b_grad;
    TOFU_TENSOR_DATA_TO(w->grad, 0, w_grad, TOFU_FLOAT);
    TOFU_TENSOR_DATA_TO(b->grad, 0, b_grad, TOFU_FLOAT);
...
void test_optimizer_sgd_momentum()
{
    ...
    tofu_optimizer* opt = tofu_optimizer_sgd_momentum_create(g, 0.1, 0.9);
    assert(opt != NULL);
    assert(opt->type == TOFU_OPTIM_SGD_MOMENTUM);
    assert(opt->num_params == 1);

    /* Manually set a gradient and do two steps */
    float w_initial;
    TOFU_TENSOR_DATA_TO(w->value, 0, w_initial, TOFU_FLOAT);

    /* First step: create gradient and step */
    w->grad = tofu_tensor_zeros(1, (int[]){1}, TOFU_FLOAT);
    float grad_val = 1.0f;
    TOFU_TENSOR_DATA_FROM(w->grad, 0, grad_val, TOFU_FLOAT);

    tofu_optimizer_step(opt);

    float w_after_step1;
    TOFU_TENSOR_DATA_TO(w->value, 0, w_after_step1, TOFU_FLOAT);
    ...
    float w_after_step2;
    TOFU_TENSOR_DATA_TO(w->value, 0, w_after_step2, TOFU_FLOAT);
...
void test_training_loop()
{
    ...
    for (int iter = 0; iter < 1; iter++) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            /* Create input */
            tofu_tensor* t_x = tofu_tensor_create(&train_x[i], 1, (int[]){1}, TOFU_FLOAT);
            tofu_graph_node* x = tofu_graph_input(g, t_x);

            /* Forward pass: pred = w * x + b */
            tofu_graph_node* wx = tofu_graph_mul(g, w, x);
            tofu_graph_node* pred = tofu_graph_add(g, wx, b);
            ...
            pred->grad = tofu_tensor_create((float[]){2.0f * error}, 1, (int[]){1}, TOFU_FLOAT);
    ...
    TOFU_TENSOR_DATA_TO(w->value, 0, final_w, TOFU_FLOAT);
    TOFU_TENSOR_DATA_TO(b->value, 0, final_b, TOFU_FLOAT);
...
void test_mlp_xor()
{
    ...
    tofu_tensor* t_b1 = tofu_tensor_create(b1_data, 1, (int[]){4}, TOFU_FLOAT);
    tofu_tensor* t_W2 = tofu_tensor_create(W2_data, 2, (int[]){4, 1}, TOFU_FLOAT);
    tofu_tensor* t_b2 = tofu_tensor_create(b2_data, 1, (int[]){1}, TOFU_FLOAT);
    ...
    for (int i = 0; i < num_samples; i++) {
        tofu_tensor* t_x = tofu_tensor_create(X_data[i], 1, (int[]){2}, TOFU_FLOAT);
        tofu_graph_node* x = tofu_graph_input(g, t_x);

        /* Forward: hidden = ReLU(x @ W1 + b1) */
        tofu_graph_node* h1 = tofu_graph_matmul(g, x, W1);
        tofu_graph_node* h1_bias = tofu_graph_add(g, h1, b1);
        tofu_graph_node* hidden = tofu_graph_relu(g, h1_bias);

        /* Forward: output = hidden @ W2 + b2 */
        tofu_graph_node* o1 = tofu_graph_matmul(g, hidden, W2);
        tofu_graph_node* output = tofu_graph_add(g, o1, b2);
    ...
    for (int iter = 0; iter < 20; iter++) {
        ...
        for (int i = 0; i < num_samples; i++) {
            ...
            /* Backward pass: grad = 2 * (pred - target) */
            output->grad = tofu_tensor_create((float[]){2.0f * error}, 1, (int[]){1}, TOFU_FLOAT);
...
void test_vit_patch_embedding()
{
     ...
     */
    tofu_graph_node* patches = tofu_graph_reshape(g, img, 2, (int[]){4, 4});
    ...
    tofu_tensor* t_W = tofu_tensor_create(W_data, 2, (int[]){patch_dim, embed_dim}, TOFU_FLOAT);
    tofu_graph_node* W = tofu_graph_param(g, t_W);

    tofu_graph_node* embeddings = tofu_graph_matmul(g, patches, W);
...
void test_vit_self_attention()
{
    ...
    tofu_tensor* t_input = tofu_tensor_create(input_data, 2, (int[]){seq_len, embed_dim}, TOFU_FLOAT);
    ...
    tofu_tensor* t_Wq = tofu_tensor_create(Wq_data, 2, (int[]){embed_dim, embed_dim}, TOFU_FLOAT);
    tofu_tensor* t_Wk = tofu_tensor_create(Wk_data, 2, (int[]){embed_dim, embed_dim}, TOFU_FLOAT);
    tofu_tensor* t_Wv = tofu_tensor_create(Wv_data, 2, (int[]){embed_dim, embed_dim}, TOFU_FLOAT);

    tofu_graph_node* Wq = tofu_graph_param(g, t_Wq);
    tofu_graph_node* Wk = tofu_graph_param(g, t_Wk);
    tofu_graph_node* Wv = tofu_graph_param(g, t_Wv);

    /* Compute Q, K, V: [4, 8] @ [8, 8] → [4, 8] */
    tofu_graph_node* Q = tofu_graph_matmul(g, input, Wq);
    tofu_graph_node* K = tofu_graph_matmul(g, input, Wk);
    tofu_graph_node* V = tofu_graph_matmul(g, input, Wv);

    /* Attention scores: Q @ K^T → [4, 4]
     * K^T: transpose [4, 8] → [8, 4]
     * Q @ K^T: [4, 8] @ [8, 4] → [4, 4]
     */
    tofu_graph_node* K_T = tofu_graph_transpose(g, K, NULL);  /* Default transpose swaps last two dims */
    tofu_graph_node* scores = tofu_graph_matmul(g, Q, K_T);

    /* Apply softmax: [4, 4] */
    tofu_graph_node* attn_weights = tofu_graph_softmax(g, scores, 1);  /* Softmax over last dim */
    ...
    tofu_tensor_free(t_Wq);
    tofu_tensor_free(t_Wk);
    tofu_tensor_free(t_Wv);
    tofu_graph_free(g);
    printf("  ✓ PASSED\n");
}
...
void test_vit_training()
{
    ...
    tofu_tensor* t_W_embed = tofu_tensor_create(W_embed_data, 2, (int[]){patch_dim, embed_dim}, TOFU_FLOAT);
    tofu_graph_node* W_embed = tofu_graph_param(g, t_W_embed);
    ...
    tofu_tensor* t_W_cls = tofu_tensor_create(W_cls_data, 2, (int[]){cls_input_dim, num_classes}, TOFU_FLOAT);
    tofu_graph_node* W_cls = tofu_graph_param(g, t_W_cls);
    ...
    for (int sample_idx = 0; sample_idx < 2; sample_idx++) {
        ...
        tofu_graph_node* patches = tofu_graph_reshape(g, img, 2, (int[]){num_patches, patch_dim});
        tofu_graph_node* embeddings = tofu_graph_matmul(g, patches, W_embed);

        /* Self-attention: Q @ K^T → softmax → @ V */
        tofu_graph_node* Q = tofu_graph_matmul(g, embeddings, Wq);
        tofu_graph_node* K = tofu_graph_matmul(g, embeddings, Wk);
        tofu_graph_node* V = tofu_graph_matmul(g, embeddings, Wv);
        tofu_graph_node* K_T = tofu_graph_transpose(g, K, NULL);
        tofu_graph_node* scores = tofu_graph_matmul(g, Q, K_T);
        tofu_graph_node* attn = tofu_graph_softmax(g, scores, 1);
        tofu_graph_node* attn_out = tofu_graph_matmul(g, attn, V);

        /* Flatten all tokens: [4, 8] → [32] */
        tofu_graph_node* flattened = tofu_graph_reshape(g, attn_out, 1, (int[]){cls_input_dim});

        /* Classification: [32] @ [32, 1] → [1] */
        tofu_graph_node* logits = tofu_graph_matmul(g, flattened, W_cls);

        /* Compute loss */
        float pred;
        TOFU_TENSOR_DATA_TO(logits->value, 0, pred, TOFU_FLOAT);
    ...
    for (int iter = 0; iter < 50; iter++) {
        ...
        for (int sample_idx = 0; sample_idx < 2; sample_idx++) {
            ...
            /* Backward pass - using tofu_tensor_create_with_values (fixed memory issue) */
            float grad_value = 2.0f * error;
            logits->grad = tofu_tensor_create_with_values(&grad_value, 1, (int[]){1});
            tofu_graph_backward(g, logits);
    ...
    tofu_tensor_free(t_W_embed);
    tofu_tensor_free(t_Wq);
    tofu_tensor_free(t_Wk);
    tofu_tensor_free(t_Wv);
    tofu_tensor_free(t_W_cls);
    tofu_optimizer_free(opt);
...
void test_system_validation()
{
    ...
    printf("\n  Recent Improvements:\n");
    printf("  ✓ Fixed: graph_free() memory corruption (use tofu_tensor_create_with_values)\n");
    printf("  ✓ Fixed: Graph accumulation (use tofu_graph_clear_ops in training loops)\n");
