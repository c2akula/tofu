...
#include "../src/tofu_tensor_broadcast_internal.h"
...
LN_TEST_START(test_broadcast_shape_calculation)
{
    ...
    t1 = tofu_tensor_zeros(2, ARR(int, 3, 3), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(2, ARR(int, 3, 3), TOFU_FLOAT);

    tofu_compute_broadcast_dims(t1, t2, &out_ndim, out_dims);
    ...
    t1 = tofu_tensor_zeros(1, ARR(int, 1), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(2, ARR(int, 2, 3), TOFU_FLOAT);
    ...
    t1 = tofu_tensor_zeros(1, ARR(int, 3), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(2, ARR(int, 2, 3), TOFU_FLOAT);
    ...
    t1 = tofu_tensor_zeros(2, ARR(int, 2, 1), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(2, ARR(int, 2, 3), TOFU_FLOAT);
    ...
    t1 = tofu_tensor_zeros(3, ARR(int, 2, 1, 4), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(3, ARR(int, 1, 3, 4), TOFU_FLOAT);
...
LN_TEST_START(test_broadcast_stride_calculation)
{
    ...
    t = tofu_tensor_zeros(2, ARR(int, 3, 3), TOFU_FLOAT);
    out_dims[0] = 3;
    out_dims[1] = 3;

    tofu_compute_broadcast_strides(t, 2, out_dims, strides);
    ...
    t = tofu_tensor_zeros(1, ARR(int, 1), TOFU_FLOAT);
    out_dims[0] = 3;
    out_dims[1] = 3;
    ...
    t = tofu_tensor_zeros(1, ARR(int, 3), TOFU_FLOAT);
    out_dims[0] = 2;
    out_dims[1] = 3;
    ...
    t = tofu_tensor_zeros(2, ARR(int, 3, 1), TOFU_FLOAT);
    out_dims[0] = 3;
    out_dims[1] = 3;
    ...
    t = tofu_tensor_zeros(2, ARR(int, 1, 3), TOFU_FLOAT);
    out_dims[0] = 2;
    out_dims[1] = 3;
    ...
    t = tofu_tensor_zeros(3, ARR(int, 2, 1, 4), TOFU_FLOAT);
    out_dims[0] = 2;
    out_dims[1] = 3;
    out_dims[2] = 4;

    tofu_compute_broadcast_strides(t, 3, out_dims, strides);
...
LN_TEST_START(test_broadcast_index_mapping)
{
    ...
    src = tofu_tensor_create(src_data, 2, ARR(int, 2, 1), TOFU_INT32);

    out_dims[0] = 2;
    out_dims[1] = 3;
    out_ndim = 2;

    tofu_compute_broadcast_strides(src, out_ndim, out_dims, strides);
...
LN_TEST_START(test_broadcast_with_ground_truth)
{
    ...
    int out_ndim;
    int out_dims[TOFU_MAXDIM];
    int strides1[TOFU_MAXDIM];
    int strides2[TOFU_MAXDIM];
    ...
    src1 = tofu_tensor_create(data1, 2, ARR(int, 2, 1), TOFU_INT32);
    src2 = tofu_tensor_create(data2, 2, ARR(int, 2, 3), TOFU_INT32);
    ...
    // Compute strides
    tofu_compute_broadcast_strides(src1, out_ndim, out_dims, strides1);
    tofu_compute_broadcast_strides(src2, out_ndim, out_dims, strides2);
...
LN_TEST_START(test_broadcast_properties)
{
    ...
    int out_dims1[TOFU_MAXDIM];
    int out_dims2[TOFU_MAXDIM];

    // Property 1: Commutativity of shape calculation
    // broadcast_shape(A, B) == broadcast_shape(B, A)
    t1 = tofu_tensor_zeros(2, ARR(int, 3, 1), TOFU_FLOAT);
    t2 = tofu_tensor_zeros(2, ARR(int, 1, 3), TOFU_FLOAT);

    tofu_compute_broadcast_dims(t1, t2, &out_ndim1, out_dims1);
    tofu_compute_broadcast_dims(t2, t1, &out_ndim2, out_dims2);
    ...
    t1 = tofu_tensor_create(src_data, 1, ARR(int, 3), TOFU_INT32);

    tofu_tensor *broadcasted = tofu_tensor_broadcast_to(t1, NULL, 2, ARR(int, 2, 3));
    ...
    tofu_tensor_free(t1);
    tofu_tensor_free_data_too(broadcasted);
}
LN_TEST_END
